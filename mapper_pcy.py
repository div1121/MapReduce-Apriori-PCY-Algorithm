...
pcy_total = 100000
word_pair_basket = [0] * pcy_total

def hashfunction(word_i,word_j):
    return hash(word_i + word_j) % pcy_total
...
for line in sys.stdin:
    ...
    for i in range(count):
        for j in range(i+1,count):
            index = hashfunction(words[i],words[j])
            word_pair_basket[index] += 1
    total_baskets += 1
...
for i in range(frequent_word_len):
    for j in range(i+1,frequent_word_len):
        ...
        index = hashfunction(frequent_word[i], frequent_word[j])
        if word_pair_basket[index] / total_baskets >= threshold:
